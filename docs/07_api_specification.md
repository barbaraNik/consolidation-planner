...
| URL пример | api/v1/orders?search=q&port=CNSHA&status=Груз готов&supplier=name&exclude_consolidated=true&page=1&page_size=50 |
...
| status | string | Нет | Фильтр по статусу. По умолчанию: «Груз готов». |
...
| HTTP Code | Код ошибки | Описание | Текст ошибки |
|---|---|---|---|
| 500 | PDF_GENERATION_ERROR / EXCEL_GENERATION_ERROR | Ошибка при создании PDF/Excel | `{"error": "Попробуйте другой формат или повторите позже"}` |
